...
template<
      CPPCMS_BOOST_PP_ENUM_PARAMS(i, typename T)
    >
struct list1
    : l_item<
          long_<1>
...
#   define MPL_AUX_LIST_TAIL(list, i, T) \
    CPPCMS_BOOST_PP_CAT(list,CPPCMS_BOOST_PP_DEC(i))< \
      CPPCMS_BOOST_PP_ENUM_SHIFTED_PARAMS(i, T) \
    > \
    /**/
...
template<
      CPPCMS_BOOST_PP_ENUM_PARAMS(i, typename T)
    >
struct CPPCMS_BOOST_PP_CAT(list,i)
    : l_item<
          long_<i>
        , T0
        ...
        >
{
    typedef CPPCMS_BOOST_PP_CAT(list,i) type;
};
