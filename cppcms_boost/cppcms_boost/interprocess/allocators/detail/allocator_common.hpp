...
#ifndef CPPCMS_BOOST_INTERPROCESS_ALLOCATOR_DETAIL_ALLOCATOR_COMMON_HPP
#define CPPCMS_BOOST_INTERPROCESS_ALLOCATOR_DETAIL_ALLOCATOR_COMMON_HPP
...
#include <cppcms_boost/interprocess/detail/utilities.hpp> //get_pointer
#include <utility>   //std::pair
#include <cppcms_boost/utility/addressof.hpp> //boost::addressof
#include <cppcms_boost/assert.hpp>   //BOOST_ASSERT
#include <cppcms_boost/interprocess/exceptions.hpp> //bad_alloc
#include <cppcms_boost/interprocess/sync/scoped_lock.hpp> //scoped_lock
#include <cppcms_boost/interprocess/containers/allocation_type.hpp> //boost::interprocess::allocation_type
...
#include <cppcms_boost/interprocess/detail/segment_manager_helper.hpp>
...
struct get_or_create_node_pool_func
{
   ...
   void operator()()
   {
      ...
      mp_node_pool =    mp_segment_manager->template find_or_construct
                        <NodePool>(cppcms_boost::interprocess::unique_instance)(mp_segment_manager);
...
struct destroy_if_last_link_func
{
   ...
   void operator()()
   {
      ...
      mp_node_pool->get_segment_manager()->template destroy<NodePool>(cppcms_boost::interprocess::unique_instance); 
   }  
...
class cache_impl
{
   ...
   multiallocation_chain cached_allocation(std::size_t n)
   {
      ...
      CPPCMS_BOOST_TRY{
         ...
         return cppcms_boost::interprocess::move(chain);
      }
      CPPCMS_BOOST_CATCH(...){
         this->cached_deallocation(cppcms_boost::interprocess::move(chain));
         CPPCMS_BOOST_RETHROW
      }
      CPPCMS_BOOST_CATCH_END
   }
   ...
   void deallocate_all_cached_nodes()
   {
      ...
      mp_node_pool->deallocate_nodes(cppcms_boost::interprocess::move(m_cached_nodes));
   }
   ...
   void priv_deallocate_n_nodes(std::size_t n)
   {
      ...
      mp_node_pool->deallocate_nodes(cppcms_boost::interprocess::move(chain));
   }
};
...
class array_allocation_impl
{
   ...
   typedef cppcms_boost::container::containers_detail::transform_multiallocation_chain
   ...
   std::pair<pointer, bool>
      allocation_command(cppcms_boost::interprocess::allocation_type command,
   ...
   {  return this->derived()->get_segment_manager()->deallocate_many(cppcms_boost::interprocess::move(chain)); }
   ...
   {  return pointer(cppcms_boost::addressof(value));  }

   //!Returns address of non mutable object.
   //!Never throws
   const_pointer address(const_reference value) const
   {  return const_pointer(cppcms_boost::addressof(value));  }
   ...
   {  CPPCMS_BOOST_ASSERT(ptr != 0); (*ptr).~value_type();  }
};
         ...
         <cached_allocator_impl<T, NodePool, Version>, T, typename NodePool::segment_manager>
{
   ...
   void deallocate_individual(multiallocation_chain chain)
   {
      typename node_pool_t::multiallocation_chain mem
         (chain.extract_multiallocation_chain());
      m_cache.cached_deallocation(cppcms_boost::interprocess::move(mem));
   }
   ...
   : public private_node_allocator_t
{
 ...
 public:
   ...
   {
      //-----------------------
      cppcms_boost::interprocess::scoped_lock<mutex_type> guard(m_header);
      //-----------------------
...
*/
   ...
   void deallocate_nodes(multiallocation_chain chain)
   {
      ...
      cppcms_boost::interprocess::scoped_lock<mutex_type> guard(m_header);
      //-----------------------
      private_node_allocator_t::deallocate_nodes(cppcms_boost::interprocess::move(chain));
   }
