...
#ifndef CPPCMS_BOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
#define CPPCMS_BOOST_INTERPROCESS_SEGMENT_MANAGER_HPP
...
#include <cppcms_boost/detail/no_exceptions_support.hpp>
#include <cppcms_boost/interprocess/detail/type_traits.hpp>

#include <cppcms_boost/interprocess/detail/transform_iterator.hpp>

#include <cppcms_boost/interprocess/detail/mpl.hpp>
#include <cppcms_boost/interprocess/detail/segment_manager_helper.hpp>
#include <cppcms_boost/interprocess/detail/named_proxy.hpp>
#include <cppcms_boost/interprocess/detail/utilities.hpp>
#include <cppcms_boost/interprocess/offset_ptr.hpp>
#include <cppcms_boost/interprocess/indexes/iset_index.hpp>
#include <cppcms_boost/interprocess/exceptions.hpp>
#include <cppcms_boost/interprocess/allocators/allocator.hpp>
#include <cppcms_boost/interprocess/smart_ptr/deleter.hpp>
...
#ifndef CPPCMS_BOOST_NO_EXCEPTIONS
#include <exception>
#endif
   ...
   :  private MemoryAlgorithm
{
   ...
   multiallocation_chain allocate_many(std::size_t elem_bytes, std::size_t num_elements)
   {
      ...
      return cppcms_boost::interprocess::move(mem);
   }
   ...
   {  MemoryAlgorithm::deallocate_many(cppcms_boost::interprocess::move(chain)); }
                           ...
                           T *reuse_ptr = 0)
   {
      std::pair<T *, bool> ret = MemoryAlgorithm::allocation_command
         ( command | cppcms_boost::interprocess::nothrow_allocation, limit_size, preferred_size, received_size
         , reuse_ptr);
      if(!(command & cppcms_boost::interprocess::nothrow_allocation) && !ret.first)
         throw bad_alloc();
      return ret;
   }
                           ...
                           void *reuse_ptr = 0, std::size_t sizeof_object = 1)
   {
      std::pair<void *, bool> ret = MemoryAlgorithm::raw_allocation_command
         ( command | cppcms_boost::interprocess::nothrow_allocation, limit_objects, preferred_objects, received_objects
...
{ 
   ...
   struct allocator
   {
      typedef cppcms_boost::interprocess::allocator<T, segment_manager> type;
   };
   ...
   struct deleter
   {
      typedef cppcms_boost::interprocess::deleter<T, segment_manager> type;
   };
   ...
   {
      cppcms_boost::interprocess::allocation_type type = ctrl_data->alloc_type();
      if(type != named_type){
   ...
   scoped_lock<rmutex> priv_get_lock(bool use_lock)
   {
      ...
      return scoped_lock<rmutex>(cppcms_boost::interprocess::move(local));
   }
