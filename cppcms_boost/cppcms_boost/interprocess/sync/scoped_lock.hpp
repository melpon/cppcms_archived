...
#ifndef CPPCMS_BOOST_INTERPROCESS_SCOPED_LOCK_HPP
#define CPPCMS_BOOST_INTERPROCESS_SCOPED_LOCK_HPP
...
#include <cppcms_boost/interprocess/sync/lock_options.hpp>
...
class scoped_lock
{
   ...
   CPPCMS_BOOST_INTERPROCESS_MOVABLE_BUT_NOT_COPYABLE(scoped_lock)
   ...
   scoped_lock(mutex_type& m, const cppcms_boost::posix_time::ptime& abs_time)
   ...
   //!   who owns it.
   scoped_lock(CPPCMS_BOOST_INTERPROCESS_RV_REF(scoped_lock) scop)
   ...
   template<class T>
   explicit scoped_lock(CPPCMS_BOOST_INTERPROCESS_RV_REF(upgradable_lock<T>) upgr
   ...
   template<class T>
   scoped_lock(CPPCMS_BOOST_INTERPROCESS_RV_REF(upgradable_lock<T>) upgr, try_to_lock_type
         , typename detail::enable_if< detail::is_same<T, Mutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
      upgradable_lock<mutex_type> &u_lock = upgr;
   ...
   template<class T>
   scoped_lock(CPPCMS_BOOST_INTERPROCESS_RV_REF(upgradable_lock<T>) upgr, cppcms_boost::posix_time::ptime &abs_time
               , typename detail::enable_if< detail::is_same<T, Mutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
      upgradable_lock<mutex_type> &u_lock = upgr;
   ...
   //!   do so in a non-blocking manner.
   template<class T>
   scoped_lock(CPPCMS_BOOST_INTERPROCESS_RV_REF(sharable_lock<T>) shar, try_to_lock_type
      , typename detail::enable_if< detail::is_same<T, Mutex> >::type * = 0)
      : mp_mutex(0), m_locked(false)
   {
   ...
   scoped_lock &operator=(CPPCMS_BOOST_INTERPROCESS_RV_REF(scoped_lock) scop)
   {  
      if(this->owns())
         this->unlock();
      m_locked = scop.owns();
