...
#ifndef CPPCMS_BOOST_INTERPROCESS_MEM_ALGO_RBTREE_BEST_FIT_HPP
#define CPPCMS_BOOST_INTERPROCESS_MEM_ALGO_RBTREE_BEST_FIT_HPP
...
#include <cppcms_boost/interprocess/mem_algo/detail/mem_algo_common.hpp>
#include <cppcms_boost/interprocess/containers/allocation_type.hpp>
#include <cppcms_boost/interprocess/containers/container/detail/multiallocation_chain.hpp>
#include <cppcms_boost/interprocess/offset_ptr.hpp>
#include <cppcms_boost/interprocess/sync/interprocess_mutex.hpp>
#include <cppcms_boost/interprocess/exceptions.hpp>
#include <cppcms_boost/interprocess/detail/utilities.hpp>
#include <cppcms_boost/interprocess/detail/min_max.hpp>
#include <cppcms_boost/interprocess/detail/math_functions.hpp>
#include <cppcms_boost/interprocess/detail/type_traits.hpp>
#include <cppcms_boost/interprocess/sync/scoped_lock.hpp>
...
#include <cppcms_boost/intrusive/set.hpp>
...
class rbtree_best_fit
{
   ...
   typedef cppcms_boost::container::containers_detail::
   ...
   multiallocation_chain allocate_many(std::size_t elem_bytes, std::size_t num_elements)
   {
      ...
      //-----------------------
      cppcms_boost::interprocess::scoped_lock<interprocess_mutex> guard(m_header);
      //-----------------------
   ...
   template<class T>
   std::pair<T *, bool>
      allocation_command  (cppcms_boost::interprocess::allocation_type command,   std::size_t limit_size,
                           std::size_t preferred_size,std::size_t &received_size, 
                           T *reuse_ptr = 0);

   std::pair<void *, bool>
     raw_allocation_command  (cppcms_boost::interprocess::allocation_type command,   std::size_t limit_object,
   ...
   std::pair<void*, bool>
      priv_allocation_command(cppcms_boost::interprocess::allocation_type command,   std::size_t limit_size,
                        std::size_t preferred_size,std::size_t &received_size, 
                        void *reuse_ptr, std::size_t sizeof_object);


   //!Real allocation algorithm with min allocation option
   std::pair<void *, bool> priv_allocate(cppcms_boost::interprocess::allocation_type command
   ...
   void* priv_expand_both_sides(cppcms_boost::interprocess::allocation_type command
                               ,std::size_t min_size
                               ,std::size_t preferred_size
                               ,std::size_t &received_size
                               ,void *reuse_ptr
   ...
   CPPCMS_BOOST_STATIC_ASSERT((Alignment >= 4));
   //Due to rbtree size optimizations, Alignment must have at least pointer alignment
   CPPCMS_BOOST_STATIC_ASSERT((Alignment >= detail::alignment_of<void_pointer>::value));
   ...
   CPPCMS_BOOST_STATIC_ASSERT((0 == (Alignment & (Alignment - std::size_t(1u)))));
   /// @endcond
   public:
...
{  
   ...
   if(priv_next_block(first_block) == old_end_block){
      std::size_t ignore;
      unique_buffer = priv_allocate(cppcms_boost::interprocess::allocate_new, 0, 0, ignore).first;
      if(!unique_buffer)
         return;
...
{  
   ...
   //-----------------------
   std::size_t ignore;
   void * ret = priv_allocate(cppcms_boost::interprocess::allocate_new, nbytes, nbytes, ignore).first;
   return ret;
}
                        ...
                        T *reuse_ptr)
{
   ...
   CPPCMS_BOOST_ASSERT(0 == ((std::size_t)ret.first % detail::alignment_of<T>::value));
...
inline std::pair<void*, bool> rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   raw_allocation_command  (cppcms_boost::interprocess::allocation_type command,   std::size_t limit_objects,
                        std::size_t preferred_objects,std::size_t &received_objects, 
                        void *reuse_ptr, std::size_t sizeof_object)
{
   if(!sizeof_object)
      return std::pair<void *, bool>(static_cast<void*>(0), 0);
   if(command & cppcms_boost::interprocess::try_shrink_in_place){
...
template<class MutexFamily, class VoidPointer, std::size_t MemAlignment>
void* rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
   priv_expand_both_sides(cppcms_boost::interprocess::allocation_type command
                         ,std::size_t min_size
                         ,std::size_t preferred_size
                         ,std::size_t &received_size
                         ,void *reuse_ptr
                         ...
                         ,std::size_t backwards_multiple)
{
   ...
   if(command & cppcms_boost::interprocess::expand_fwd){
   ...
   if(backwards_multiple){
      CPPCMS_BOOST_ASSERT(0 == (min_size       % backwards_multiple));
      CPPCMS_BOOST_ASSERT(0 == (preferred_size % backwards_multiple));
   }

   if(command & cppcms_boost::interprocess::expand_bwd){
      //Obtain the real size of the block
   ...
   deallocate_many(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::multiallocation_chain chain)
{
   ...
   cppcms_boost::interprocess::scoped_lock<interprocess_mutex> guard(m_header);
   //-----------------------
   algo_impl_t::deallocate_many(this, cppcms_boost::interprocess::move(chain));
}
               ...
               ,std::size_t backwards_multiple)
{
   ...
   if(command & cppcms_boost::interprocess::shrink_in_place){
      bool success = 
   ...
   //Expand in place
   if(reuse_ptr && (command & (cppcms_boost::interprocess::expand_fwd | cppcms_boost::interprocess::expand_bwd))){
   ...
   if(command & cppcms_boost::interprocess::allocate_new){
