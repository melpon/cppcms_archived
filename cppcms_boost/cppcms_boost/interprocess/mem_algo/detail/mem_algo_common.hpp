...
#ifndef CPPCMS_BOOST_INTERPROCESS_DETAIL_MEM_ALGO_COMMON_HPP
#define CPPCMS_BOOST_INTERPROCESS_DETAIL_MEM_ALGO_COMMON_HPP
...
class memory_algorithm_common
{
   ...
   {
      (void)uint_ptr;
      CPPCMS_BOOST_ASSERT(uint_ptr % Alignment == 0);
   }
   ...
   {
      return this_type::priv_deallocate_many(memory_algo, cppcms_boost::interprocess::move(chain));
   }
      ...
      (MemoryAlgorithm *memory_algo, std::size_t nbytes, std::size_t alignment)
   {
      ...
      if ((alignment & (alignment - std::size_t(1u))) != 0){
         ...
         CPPCMS_BOOST_ASSERT((alignment & (alignment - std::size_t(1u))) == 0);
         return 0;
      }
      ...
      if(alignment <= Alignment){
         return memory_algo->priv_allocate
            (cppcms_boost::interprocess::allocate_new, nbytes, nbytes, real_size).first;
      }
      ...
      void *buffer = memory_algo->priv_allocate
         (cppcms_boost::interprocess::allocate_new, request, request, real_size).first;
      if(!buffer){
         return 0;
      }
         ...
         if(old_size >= (first_min_units + MinBlockUnits)){
            ...
            CPPCMS_BOOST_ASSERT(second->m_size >= MinBlockUnits);
      ...
      if((old_size - first->m_size) >= (second_min_units + MinBlockUnits)){
         ...
         CPPCMS_BOOST_ASSERT(third->m_size >= MinBlockUnits);
         memory_algo->priv_mark_new_allocated_block(second);
      ...
      ,std::size_t &received_size)
   {
      ...
      CPPCMS_BOOST_ASSERT(memory_algo->priv_is_allocated_block(block));
      ...
      ,std::size_t &received_size)
   {
      ...
      CPPCMS_BOOST_ASSERT(block->m_size >= BlockCtrlUnits);
      ...
      CPPCMS_BOOST_ASSERT(new_block->m_size >= BlockCtrlUnits);
      ...
      , std::size_t sizeof_element)
   {
      ...
      while(low_idx < n_elements){
         ...
         std::pair<void *, bool> ret = memory_algo->priv_allocate
            (cppcms_boost::interprocess::allocate_new, min_allocation, total_bytes, received_size, 0);
         if(!ret.first){
            break;
         }
...
//         block_ctrl *prev_block = 0;
         while(total_used_units < received_units){
            ...
            if((low_idx + 1) == n_elements ||
               ...
               if((received_units - total_used_units) >= (elem_units + MemoryAlgorithm::BlockCtrlUnits)){
                  ...
                  CPPCMS_BOOST_ASSERT(shrink_ok);
                  //Some sanity checks
                  CPPCMS_BOOST_ASSERT(shrunk_request == shrunk_received);
                  CPPCMS_BOOST_ASSERT(elem_units == ((shrunk_request-UsableByPreviousChunk)/Alignment + AllocatedCtrlUnits));
                  //"new_block->m_size" must have been reduced to elem_units by "shrink"
                  CPPCMS_BOOST_ASSERT(new_block->m_size == elem_units);
         ...
         //Sanity check
         CPPCMS_BOOST_ASSERT(total_used_units == received_units);
      }
      
      if(low_idx != n_elements){
         priv_deallocate_many(memory_algo, cppcms_boost::interprocess::move(chain));
      }
