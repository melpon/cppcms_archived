...
#ifndef CPPCMS_BOOST_INTRUSIVE_DETAIL_UTILITIES_HPP
#define CPPCMS_BOOST_INTRUSIVE_DETAIL_UTILITIES_HPP

#include <cppcms_boost/intrusive/detail/config_begin.hpp>
#include <cppcms_boost/intrusive/detail/pointer_to_other.hpp>
#include <cppcms_boost/intrusive/detail/parent_from_member.hpp>
#include <cppcms_boost/intrusive/detail/ebo_functor_holder.hpp>
...
#include <cppcms_boost/intrusive/detail/is_stateful_value_traits.hpp>
...
struct internal_base_hook_bool
{
   ...
   template <class U> static two_or_three<U::cppcms_boost_intrusive_tags::is_base_hook> test (int);
   ...
   :  private detail::ebo_functor_holder<F>
{
   ...
   node_ptr operator()(const node &to_clone)
   {
      ...
      if(safemode_or_autounlink)
         CPPCMS_BOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::unique(n));
      return n;
   }
...
{  //If this assertion raises, you might have destroyed an object
   ...
   (void)hook; CPPCMS_BOOST_INTRUSIVE_SAFE_HOOK_DESTRUCTOR_ASSERT(!hook.is_linked());
}
...
struct base_hook_traits
{
   ...
   typedef typename cppcms_boost::pointer_to_other<node_ptr, T>::type       pointer;
   typedef typename cppcms_boost::pointer_to_other<node_ptr, const T>::type const_pointer;
...
struct member_hook_traits
{
   ...
   typedef typename hook_type::cppcms_boost_intrusive_tags::node_traits     node_traits;
   ...
   typedef typename std::iterator_traits<pointer>::reference         reference;
   typedef typename std::iterator_traits<const_pointer>::reference   const_reference;
   static const link_mode_type link_mode = Hook::cppcms_boost_intrusive_tags::link_mode;
...
inline std::size_t floor_log2 (std::size_t x)
{
   ...
   CPPCMS_BOOST_STATIC_ASSERT(Size_t_Bits_Power_2);
...
{
   cppcms_boost::uint32_t * exp_ptr =
      static_cast<cppcms_boost::uint32_t *>(static_cast<void*>(&val));
   cppcms_boost::uint32_t x = *exp_ptr;
...
{
   static const cppcms_boost::uint32_t value = 0xb504f334;
...
{
   static const cppcms_boost::uint64_t value = 0xb504f333f9de6484ull;
...
struct node_to_value
   :  public detail::select_constptr
      < typename cppcms_boost::pointer_to_other
