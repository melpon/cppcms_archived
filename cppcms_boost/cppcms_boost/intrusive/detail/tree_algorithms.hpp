...
#ifndef CPPCMS_BOOST_INTRUSIVE_TREE_ALGORITHMS_HPP
#define CPPCMS_BOOST_INTRUSIVE_TREE_ALGORITHMS_HPP
...
class tree_algorithms
{
   ...
   {
      return node_ptr(const_cast<node*>(::cppcms_boost::intrusive::detail::get_pointer(ptr)));
   }
   /// @endcond
      ...
      (node_ptr header, node_ptr new_node, const insert_commit_data &commit_data)
   {
      ...
      CPPCMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(commit_data.node != 0);
   ...
   static node_ptr subtree_to_vine(node_ptr old_root, std::size_t *plen = 0)
   {
      ...
      CPPCMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(super_root);
   ...
   {
      CPPCMS_BOOST_INTRUSIVE_INVARIANT_ASSERT((!inited(node)));
      node_ptr x = NodeTraits::get_parent(node);
      if(x){
         while(!is_header(x)){
            x = NodeTraits::get_parent(x);
