...
#ifndef CPPCMS_BOOST_INTRUSIVE_SLIST_HPP
#define CPPCMS_BOOST_INTRUSIVE_SLIST_HPP
...
#include <cppcms_boost/intrusive/slist_hook.hpp>
#include <cppcms_boost/intrusive/circular_slist_algorithms.hpp>
#include <cppcms_boost/intrusive/linear_slist_algorithms.hpp>
#include <cppcms_boost/intrusive/detail/pointer_to_other.hpp>
#include <cppcms_boost/intrusive/detail/clear_on_destructor_base.hpp>
#include <cppcms_boost/intrusive/link_mode.hpp>
#include <cppcms_boost/intrusive/options.hpp>
#include <cppcms_boost/intrusive/detail/utilities.hpp>
#include <iterator>
#include <functional>
#include <algorithm>
...
#if defined(CPPCMS_BOOST_INTRUSIVE_DOXYGEN_INVOKED)
   ...
   :  private detail::clear_on_destructor_base<slist_impl<Config> >
{
   ...
   CPPCMS_BOOST_STATIC_ASSERT(!(constant_time_size && ((int)real_value_traits::link_mode == (int)auto_unlink)));
   //Linear singly linked lists are incompatible with auto-unlink hooks!
   CPPCMS_BOOST_STATIC_ASSERT(!(linear && ((int)real_value_traits::link_mode == (int)auto_unlink)));
   //A list with cached last node is incompatible with auto-unlink hooks!
   CPPCMS_BOOST_STATIC_ASSERT(!(cache_last && ((int)real_value_traits::link_mode == (int)auto_unlink)));
   ...
   void push_front(reference value) 
   {
      ...
      if(safemode_or_autounlink)
         CPPCMS_BOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::inited(to_insert));
      if(cache_last){
         if(this->empty()){
   ...
   {
      CPPCMS_BOOST_STATIC_ASSERT((cache_last != 0));
   ...
   iterator insert_after(const_iterator prev_p, reference value)
   {
      ...
      if(safemode_or_autounlink)
         CPPCMS_BOOST_INTRUSIVE_SAFE_HOOK_DEFAULT_ASSERT(node_algorithms::inited(n));
   ...
   {
      CPPCMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(std::distance(++const_iterator(before_first), last) == difference_type(n));
      if(safemode_or_autounlink){
   ...
   {
      CPPCMS_BOOST_STATIC_ASSERT(((!cache_last)&&(!constant_time_size)&&(!stateful_value_traits)));
      const_iterator it(prev);
      ++it;
      node_ptr to_erase(it.pointed_node());
      ++it;
   ...
   {
      if(n){
         CPPCMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(std::distance(before_first, before_last) == n);
         this->priv_splice_after
            (prev_pos.pointed_node(), x, before_first.pointed_node(), before_last.pointed_node());
         if(constant_time_size){
            this->priv_size_traits().set_size(this->priv_size_traits().get_size() + n);
   ...
   void sort(Predicate p)
   {
      if (node_traits::get_next(node_traits::get_next(this->get_root_node()))
         ...
         while(!this->empty()){
            ...
            CPPCMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(counter[i].empty());
   ...
   {
      CPPCMS_BOOST_STATIC_ASSERT((!stateful_value_traits));
   ...
   {
      if(n){
         CPPCMS_BOOST_INTRUSIVE_INVARIANT_ASSERT(std::distance(iterator(first, this), iterator(before_last, this))+1 == n);
   ...
   static slist_impl &priv_container_from_end_iterator(const const_iterator &end_iterator)
   {
      ...
      CPPCMS_BOOST_STATIC_ASSERT(!linear);
...
#if defined(CPPCMS_BOOST_INTRUSIVE_DOXYGEN_INVOKED) || defined(CPPCMS_BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
template<class T, class ...Options>
#else
...
struct make_slist
{
   ...
   typedef typename pack_options
      < slist_defaults<T>, 
         #if !defined(CPPCMS_BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
         O1, O2, O3, O4, O5
         #else
         Options...
         #endif
...
#ifndef CPPCMS_BOOST_INTRUSIVE_DOXYGEN_INVOKED
      ...
      >::type
{
   ...
   CPPCMS_BOOST_STATIC_ASSERT((detail::is_same<typename real_value_traits::value_type, T>::value));
   public:
