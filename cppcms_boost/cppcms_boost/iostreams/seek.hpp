...
#ifndef CPPCMS_BOOST_IOSTREAMS_SEEK_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_SEEK_HPP_INCLUDED
...
#include <cppcms_boost/iostreams/detail/ios.hpp>       // streamsize, seekdir, openmode.
...
seek( T& t, stream_offset off, CPPCMS_BOOST_IOS::seekdir way,
      CPPCMS_BOOST_IOS::openmode which = CPPCMS_BOOST_IOS::in | CPPCMS_BOOST_IOS::out )
{ 
...
inline std::streampos
seek( T& t, Device& dev, stream_offset off, CPPCMS_BOOST_IOS::seekdir way,
      CPPCMS_BOOST_IOS::openmode which = CPPCMS_BOOST_IOS::in | CPPCMS_BOOST_IOS::out )
{ 
    using namespace detail;
...
struct seek_impl_basic_ios {
    ...
    static std::streampos seek( T& t, stream_offset off,
                                CPPCMS_BOOST_IOS::seekdir way,
                                CPPCMS_BOOST_IOS::openmode which )
    {
        if ( way == CPPCMS_BOOST_IOS::beg &&
                                ...
                                CPPCMS_BOOST_IOS::openmode which )
    {
        if ( way == CPPCMS_BOOST_IOS::beg &&
             ( off < integer_traits<std::streamoff>::const_min ||
               off > integer_traits<std::streamoff>::const_max ) )
        {
            return t.CPPCMS_BOOST_IOSTREAMS_PUBSEEKPOS(offset_to_position(off));
        } else {
            return t.CPPCMS_BOOST_IOSTREAMS_PUBSEEKOFF(off, way, which);
        }
    }
};
