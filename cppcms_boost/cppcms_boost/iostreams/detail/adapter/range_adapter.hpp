...
#ifndef CPPCMS_BOOST_IOSTREAMS_DETAIL_RANGE_ADAPTER_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_DETAIL_RANGE_ADAPTER_HPP_INCLUDED
...
#include <cppcms_boost/detail/iterator.hpp>             // boost::iterator_traits.
...
private:
    ...
    typedef cppcms_boost::detail::iterator_traits<iterator>  iter_traits;
...
struct range_adapter_impl<std::random_access_iterator_tag> {
          ...
          CPPCMS_BOOST_IOS::seekdir way )
    {
        using namespace std;
        switch (way) {
        case CPPCMS_BOOST_IOS::beg:
            if (off > last - first || off < 0) throw bad_seek();
            cur = first + off;
            break;
        case CPPCMS_BOOST_IOS::cur:
            {
