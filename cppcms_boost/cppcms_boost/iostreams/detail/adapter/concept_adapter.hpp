...
#ifndef CPPCMS_BOOST_IOSTREAMS_DETAIL_CONCEPT_ADAPTER_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_DETAIL_CONCEPT_ADAPTER_HPP_INCLUDED

#include <cppcms_boost/config.hpp>                             // SFINAE.
...
#include <cppcms_boost/iostreams/detail/dispatch.hpp>
...
public:
    ...
    { CPPCMS_BOOST_STATIC_ASSERT(is_std_io<T>::value); }
    ...
    std::streampos seek( stream_offset off, CPPCMS_BOOST_IOS::seekdir way,
                         CPPCMS_BOOST_IOS::openmode which, Device* dev )
    { return any_impl::seek(t_, dev, off, way, which); }

    void close(CPPCMS_BOOST_IOS::openmode which)
    { this->close(which, (basic_null_device<char_type, seekable>*) 0); }

    template<typename Device>
    void close(CPPCMS_BOOST_IOS::openmode which, Device* dev)
    { any_impl::close(t_, dev, which); }

    bool flush( CPPCMS_BOOST_IOSTREAMS_BASIC_STREAMBUF(char_type,
                CPPCMS_BOOST_IOSTREAMS_CHAR_TRAITS(char_type))* sb )
    { 
        bool result = any_impl::flush(t_, sb);
        if (sb && sb->CPPCMS_BOOST_IOSTREAMS_PUBSYNC() == -1)
            result = false;
        return result;
    }
...
struct device_wrapper_impl<any_tag> {
    ...
    seek( Device& dev, Dummy*, stream_offset off, 
          CPPCMS_BOOST_IOS::seekdir way, CPPCMS_BOOST_IOS::openmode which )
    { 
    ...
    template<typename Device>
    static std::streampos 
    seek( Device&, stream_offset, CPPCMS_BOOST_IOS::seekdir, 
          CPPCMS_BOOST_IOS::openmode, any_tag )
    { 
        throw cant_seek(); 
    }
    ...
    template<typename Device, typename Dummy>
    static void close(Device& dev, Dummy*, CPPCMS_BOOST_IOS::openmode which)
...
struct flt_wrapper_impl<any_tag> {
    ...
    seek( Filter&, Device*, stream_offset,
          CPPCMS_BOOST_IOS::seekdir, CPPCMS_BOOST_IOS::openmode, any_tag )
    { throw cant_seek(); }
    ...
    static std::streampos
    seek( Filter& f, Device* dev, stream_offset off,
          CPPCMS_BOOST_IOS::seekdir way, CPPCMS_BOOST_IOS::openmode,
    ...
    template<typename Filter, typename Device>
    static void close(Filter& f, Device* dev, CPPCMS_BOOST_IOS::openmode which)
