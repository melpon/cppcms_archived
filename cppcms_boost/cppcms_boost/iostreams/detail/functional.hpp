...
#ifndef CPPCMS_BOOST_IOSTREAMS_DETAIL_FUNCTIONAL_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_DETAIL_FUNCTIONAL_HPP_INCLUDED
...
public:
    ...
    device_close_operation(T& t, CPPCMS_BOOST_IOS::openmode which) 
        : t_(t), which_(which) 
        { }
    void operator()() const { cppcms_boost::iostreams::close(t_, which_); }
private:
    device_close_operation& operator=(const device_close_operation&);
    T&                   t_;
    CPPCMS_BOOST_IOS::openmode  which_;
};
...
public:
    typedef void result_type;
    filter_close_operation(T& t, Sink& snk, CPPCMS_BOOST_IOS::openmode which)
        : t_(t), snk_(snk), which_(which)
        { }
    void operator()() const { cppcms_boost::iostreams::close(t_, snk_, which_); }
private:
...
call_close(T& t, CPPCMS_BOOST_IOS::openmode which) 
{ return device_close_operation<T>(t, which); }

template<typename T, typename Sink>
filter_close_operation<T, Sink> 
call_close(T& t, Sink& snk, CPPCMS_BOOST_IOS::openmode which) 
...
public:
    typedef void result_type;
    member_close_operation(T& t, CPPCMS_BOOST_IOS::openmode which) 
        : t_(t), which_(which) 
        { }
...
template<typename T>
member_close_operation<T> call_member_close(T& t, CPPCMS_BOOST_IOS::openmode which) 
