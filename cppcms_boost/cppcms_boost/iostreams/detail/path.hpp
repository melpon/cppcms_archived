...
#ifndef CPPCMS_BOOST_IOSTREAMS_DETAIL_PATH_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_DETAIL_PATH_HPP_INCLUDED
...
public:
    ...
    explicit path(const Path& p)
    {
        ...
        init(p, cppcms_boost::type<string_type>());
    }
...
private:
    ...
    template<typename Path>
    void init(const Path& p, cppcms_boost::type<std::string>)
    {
    ...
    template<typename Path>
    void init(const Path& p, cppcms_boost::type<std::wstring>)
    {
        narrow_.clear();
