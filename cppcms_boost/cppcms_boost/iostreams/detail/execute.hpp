...
#ifndef CPPCMS_BOOST_IOSTREAMS_DETAIL_EXECUTE_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_DETAIL_EXECUTE_HPP_INCLUDED
...
#include <cppcms_boost/iostreams/detail/config/limits.hpp>   // MAX_EXECUTE_ARITY
...
#include <cppcms_boost/utility/result_of.hpp>
...
template< typename Op, 
          typename Result = // VC6.5 workaround.
              #if !defined(CPPCMS_BOOST_NO_RESULT_OF) && \
                  !CPPCMS_BOOST_WORKAROUND(__BORLANDC__, CPPCMS_BOOST_TESTED_AT(0x592))
                  typename cppcms_boost::result_of<Op()>::type
              #else
                  CPPCMS_BOOST_DEDUCED_TYPENAME Op::result_type
              #endif
          >
...
#define CPPCMS_BOOST_PP_LOCAL_MACRO(n) \
   template<typename Op, CPPCMS_BOOST_PP_ENUM_PARAMS(n, typename C)> \
   typename execute_traits<Op>::result_type \
   execute_all(Op op, CPPCMS_BOOST_PP_ENUM_BINARY_PARAMS(n, C, c)) \
   { \
       typename execute_traits<Op>::result_type r; \
       try { \
           r = cppcms_boost::iostreams::detail::execute_all( \
                   op CPPCMS_BOOST_PP_COMMA_IF(CPPCMS_BOOST_PP_DEC(n)) \
                   CPPCMS_BOOST_PP_ENUM_PARAMS(CPPCMS_BOOST_PP_DEC(n), c) \
               ); \
       } catch (...) { \
           try { \
               CPPCMS_BOOST_PP_CAT(c, CPPCMS_BOOST_PP_DEC(n))(); \
           } catch (...) { } \
           throw; \
       } \
       CPPCMS_BOOST_PP_CAT(c, CPPCMS_BOOST_PP_DEC(n))(); \
...
#define CPPCMS_BOOST_PP_LOCAL_LIMITS (1, CPPCMS_BOOST_IOSTREAMS_MAX_EXECUTE_ARITY)
...
Op execute_foreach(InIt first, InIt last, Op op)
{
    ...
    try {
        ...
        try {
            ++first;
            cppcms_boost::iostreams::detail::execute_foreach(first, last, op);
        } catch (...) { }
        throw;
    }
    ++first;
    return cppcms_boost::iostreams::detail::execute_foreach(first, last, op);
}
