...
#ifndef CPPCMS_BOOST_IOSTREAMS_DETAIL_SELECT_BY_SIZE_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_DETAIL_SELECT_BY_SIZE_HPP_INCLUDED
...
#define SELECT_BY_SIZE_SPEC(n) \
    namespace cppcms_boost { namespace iostreams { namespace detail { \
      static const int CPPCMS_BOOST_PP_CAT(sizeof_case_, n) = sizeof(case_<n - 1>); \
      template<> \
      struct select_by_size< CPPCMS_BOOST_PP_CAT(sizeof_case_, n) > { \
          struct type { CPPCMS_BOOST_STATIC_CONSTANT(int, value = n - 1); }; \
          CPPCMS_BOOST_STATIC_CONSTANT(int, value = type::value); \
      }; \
    } } } \
    /**/
...
#define CPPCMS_BOOST_PP_LOCAL_MACRO(n) SELECT_BY_SIZE_SPEC(n)
#define CPPCMS_BOOST_PP_LOCAL_LIMITS (0, 20)
#include CPPCMS_BOOST_PP_LOCAL_ITERATE()
#undef CPPCMS_BOOST_PP_LOCAL_MACRO

//--------------Definition of SELECT_BY_SIZE----------------------------------//

#define CPPCMS_BOOST_SELECT_BY_SIZE(type_, name, expr) \
    CPPCMS_BOOST_STATIC_CONSTANT( \
        unsigned, \
        CPPCMS_BOOST_PP_CAT(cppcms_boost_select_by_size_temp_, name) = sizeof(expr) \
    ); \
    CPPCMS_BOOST_STATIC_CONSTANT( \
        type_, \
        name = \
            ( ::cppcms_boost::iostreams::detail::select_by_size< \
                CPPCMS_BOOST_PP_CAT(cppcms_boost_select_by_size_temp_, name) \
              >::value ) \
    ) \
    /**/
...
#if CPPCMS_BOOST_SELECT_BY_SIZE_MAX_CASE > SELECT_BY_SIZE_MAX_SPECIALIZED

#define CPPCMS_BOOST_PP_LOCAL_MACRO(n) SELECT_BY_SIZE_SPEC(n)
#define CPPCMS_BOOST_PP_LOCAL_LIMITS \
    (SELECT_BY_SIZE_MAX_SPECIALIZED, CPPCMS_BOOST_SELECT_BY_SIZE_MAX_CASE) \
    /**/
#include CPPCMS_BOOST_PP_LOCAL_ITERATE()
#undef CPPCMS_BOOST_PP_LOCAL_MACRO
#undef SELECT_BY_SIZE_MAX_SPECIALIZED
#define SELECT_BY_SIZE_MAX_SPECIALIZED CPPCMS_BOOST_SELECT_BY_SIZE_MAX_CASE
