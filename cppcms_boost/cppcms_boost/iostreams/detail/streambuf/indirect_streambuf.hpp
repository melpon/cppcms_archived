...
#ifndef CPPCMS_BOOST_IOSTREAMS_DETAIL_INDIRECT_STREAMBUF_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_DETAIL_INDIRECT_STREAMBUF_HPP_INCLUDED

#include <algorithm>                             // min, max.
#include <cassert>
#include <exception>
#include <typeinfo>
#include <cppcms_boost/config.hpp>                      // Member template friends.
#include <cppcms_boost/detail/workaround.hpp>
#include <cppcms_boost/iostreams/constants.hpp>
#include <cppcms_boost/iostreams/detail/adapter/concept_adapter.hpp>
#include <cppcms_boost/iostreams/detail/buffer.hpp>
#include <cppcms_boost/iostreams/detail/config/wide_streams.hpp>
#include <cppcms_boost/iostreams/detail/double_object.hpp> 
...
// Must come last.
#include <cppcms_boost/iostreams/detail/config/disable_warnings.hpp>  // MSVC, BCC 5.x
...
public:
    ...
    void open(const T& t CPPCMS_BOOST_IOSTREAMS_PUSH_PARAMS());
    bool is_open() const;
    void close();
...
#ifndef CPPCMS_BOOST_IOSTREAMS_NO_LOCALE
...
void indirect_streambuf<T, Tr, Alloc, Mode>::close()
{
    ...
    base_type* self = this;
    detail::execute_all(
        detail::call_member_close(*self, CPPCMS_BOOST_IOS::in),
...
{
    if ( gptr() != 0 && way == CPPCMS_BOOST_IOS::cur && which == CPPCMS_BOOST_IOS::in && 
         eback() - gptr() <= off && off <= egptr() - gptr() ) 
    {   // Small seek optimization
        gbump(off);
        return obj().seek(0, CPPCMS_BOOST_IOS::cur, CPPCMS_BOOST_IOS::in, next_) -
               static_cast<off_type>(egptr() - gptr());
    }
    if (pptr() != 0) 
        this->CPPCMS_BOOST_IOSTREAMS_PUBSYNC(); // sync() confuses VisualAge 6.
    if (way == CPPCMS_BOOST_IOS::cur && gptr())
...
inline void indirect_streambuf<T, Tr, Alloc, Mode>::close_impl
    (CPPCMS_BOOST_IOS::openmode which)
{
    if (which == CPPCMS_BOOST_IOS::in && is_convertible<Mode, input>::value) {
        setg(0, 0, 0);
    }
    if (which == CPPCMS_BOOST_IOS::out && is_convertible<Mode, output>::value) {
        sync();
        setp(0, 0);
    }
    if ( !is_convertible<category, dual_use>::value ||
         is_convertible<Mode, input>::value == (which == CPPCMS_BOOST_IOS::in) )
    {
...
#include <cppcms_boost/iostreams/detail/config/enable_warnings.hpp> // MSVC, BCC 5.x
