...
#ifndef CPPCMS_BOOST_IOSTREAMS_DETAIL_DIRECT_STREAMBUF_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_DETAIL_DIRECT_STREAMBUF_HPP_INCLUDED
...
#include <cppcms_boost/iostreams/detail/char_traits.hpp>  // member template friends.
#include <cppcms_boost/iostreams/detail/config/wide_streams.hpp>
#include <cppcms_boost/iostreams/detail/error.hpp>
#include <cppcms_boost/iostreams/detail/execute.hpp>
#include <cppcms_boost/iostreams/detail/functional.hpp>
#include <cppcms_boost/iostreams/detail/ios.hpp>
#include <cppcms_boost/iostreams/detail/optional.hpp>
#include <cppcms_boost/iostreams/detail/streambuf.hpp>
#include <cppcms_boost/iostreams/detail/streambuf/linked_streambuf.hpp>
...
class direct_streambuf 
    : public linked_streambuf<CPPCMS_BOOST_DEDUCED_TYPENAME char_type_of<T>::type, Tr>
{
public:
...
private:
    ...
    typedef CPPCMS_BOOST_IOSTREAMS_BASIC_STREAMBUF(
...
protected:
#if !CPPCMS_BOOST_WORKAROUND(__GNUC__, == 2)
    CPPCMS_BOOST_IOSTREAMS_USING_PROTECTED_STREAMBUF_MEMBERS(base_type)
#endif
    direct_streambuf();
    ...
    // Declared in linked_streambuf.
    void close_impl(CPPCMS_BOOST_IOS::openmode m);
    const std::type_info& component_type() const { return typeid(T); }
    void* component_impl() { return component(); } 
#ifdef CPPCMS_BOOST_IOSTREAMS_NO_STREAM_TEMPLATES
    public:
#endif
    ...
    pos_type seekoff( off_type off, CPPCMS_BOOST_IOS::seekdir way,
                      CPPCMS_BOOST_IOS::openmode which );
    pos_type seekpos(pos_type sp, CPPCMS_BOOST_IOS::openmode which);
private:
    pos_type seek_impl( stream_offset off, CPPCMS_BOOST_IOS::seekdir way,
                        CPPCMS_BOOST_IOS::openmode which );
...
{ 
    base_type* self = this;
    detail::execute_all( detail::call_member_close(*self, CPPCMS_BOOST_IOS::in),
                         detail::call_member_close(*self, CPPCMS_BOOST_IOS::out),
...
{
    using namespace std;
    if (!obeg_) throw CPPCMS_BOOST_IOSTREAMS_FAILURE("no write access");
    if (!pptr()) init_put_area();
    if (!traits_type::eq_int_type(c, traits_type::eof())) {
        if (pptr() == oend_)
            throw CPPCMS_BOOST_IOSTREAMS_FAILURE("write area exhausted");
...
direct_streambuf<T, Tr>::seekoff
    (off_type off, CPPCMS_BOOST_IOS::seekdir way, CPPCMS_BOOST_IOS::openmode which)
{ return seek_impl(off, way, which); }

template<typename T, typename Tr>
inline typename direct_streambuf<T, Tr>::pos_type
direct_streambuf<T, Tr>::seekpos
    (pos_type sp, CPPCMS_BOOST_IOS::openmode which)
{ 
    return seek_impl(position_to_offset(sp), CPPCMS_BOOST_IOS::beg, which);
}

template<typename T, typename Tr>
void direct_streambuf<T, Tr>::close_impl(CPPCMS_BOOST_IOS::openmode which)
{
    if (which == CPPCMS_BOOST_IOS::in && ibeg_ != 0) {
        setg(0, 0, 0);
        ibeg_ = iend_ = 0;
    }
    if (which == CPPCMS_BOOST_IOS::out && obeg_ != 0) {
        sync();
        setp(0, 0);
        obeg_ = oend_ = 0;
    }
    cppcms_boost::iostreams::close(*storage_, which);
}

template<typename T, typename Tr>
typename direct_streambuf<T, Tr>::pos_type direct_streambuf<T, Tr>::seek_impl
    (stream_offset off, CPPCMS_BOOST_IOS::seekdir way, CPPCMS_BOOST_IOS::openmode which)
{
    using namespace std;
    CPPCMS_BOOST_IOS::openmode both = CPPCMS_BOOST_IOS::in | CPPCMS_BOOST_IOS::out;
    ...
    if (one || ((which & CPPCMS_BOOST_IOS::in) != 0 && ibeg_ != 0)) {
        if (!gptr()) setg(ibeg_, ibeg_, iend_);
        ptrdiff_t next = 0;
        switch (way) {
        case CPPCMS_BOOST_IOS::beg: next = off; break;
        case CPPCMS_BOOST_IOS::cur: next = (gptr() - ibeg_) + off; break;
        case CPPCMS_BOOST_IOS::end: next = (iend_ - ibeg_) + off; break;
        default: assert(0);
        }
    ...
    if (!one && (which & CPPCMS_BOOST_IOS::out) != 0 && obeg_ != 0) {
        if (!pptr()) setp(obeg_, oend_);
        ptrdiff_t next = 0;
        switch (way) {
        case CPPCMS_BOOST_IOS::beg: next = off; break;
        case CPPCMS_BOOST_IOS::cur: next = (pptr() - obeg_) + off; break;
        case CPPCMS_BOOST_IOS::end: next = (oend_ - obeg_) + off; break;
        default: assert(0);
        }
