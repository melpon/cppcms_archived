...
#ifndef CPPCMS_BOOST_IOSTREAMS_TEE_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_TEE_HPP_INCLUDED
...
#include <cassert>
#include <cppcms_boost/config.hpp>  // BOOST_DEDUCE_TYPENAME.
#include <cppcms_boost/iostreams/categories.hpp>
#include <cppcms_boost/iostreams/detail/adapter/device_adapter.hpp>
#include <cppcms_boost/iostreams/detail/adapter/filter_adapter.hpp>
#include <cppcms_boost/iostreams/detail/call_traits.hpp>
#include <cppcms_boost/iostreams/detail/execute.hpp>
#include <cppcms_boost/iostreams/detail/functional.hpp>  // call_close_all 
...
public:
    ...
    CPPCMS_BOOST_STATIC_ASSERT(is_device<Device>::value);
    CPPCMS_BOOST_STATIC_ASSERT((
        is_convertible< // Using mode_of causes failures on VC6-7.0.
            CPPCMS_BOOST_DEDUCED_TYPENAME iostreams::category_of<Device>::type, output
        >::value
    ));
    ...
    void close(Next&, CPPCMS_BOOST_IOS::openmode)
    { 
...
CPPCMS_BOOST_IOSTREAMS_PIPABLE(tee_filter, 1)
...
public:
    ...
    typedef typename
            ...
            >::type                                    mode;
    CPPCMS_BOOST_STATIC_ASSERT(is_device<Device>::value);
    CPPCMS_BOOST_STATIC_ASSERT(is_device<Sink>::value);
    CPPCMS_BOOST_STATIC_ASSERT((
        is_same<
            char_type, 
            CPPCMS_BOOST_DEDUCED_TYPENAME char_type_of<Sink>::type
        >::value
    ));
    CPPCMS_BOOST_STATIC_ASSERT((
        is_convertible<
            CPPCMS_BOOST_DEDUCED_TYPENAME iostreams::category_of<Sink>::type, 
            output
        >::value
    ));
    struct category
    ...
    std::streamsize read(char_type* s, std::streamsize n)
    {
        CPPCMS_BOOST_STATIC_ASSERT((
            is_convertible<
                CPPCMS_BOOST_DEDUCED_TYPENAME iostreams::category_of<Device>::type, input
            >::value
        ));
