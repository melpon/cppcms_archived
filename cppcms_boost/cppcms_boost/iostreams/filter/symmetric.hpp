...
#ifndef CPPCMS_BOOST_IOSTREAMS_SYMMETRIC_FILTER_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_SYMMETRIC_FILTER_HPP_INCLUDED
...
#include <cppcms_boost/iostreams/char_traits.hpp>
...
#include <cppcms_boost/iostreams/operations.hpp>       // read, write.
...
template< typename SymmetricFilter,
          typename Alloc =
              std::allocator<
                  CPPCMS_BOOST_DEDUCED_TYPENAME char_type_of<SymmetricFilter>::type
              > >
class symmetric_filter {
public:
    typedef typename char_type_of<SymmetricFilter>::type      char_type;
    typedef CPPCMS_BOOST_IOSTREAMS_CHAR_TRAITS(char_type)            traits_type;
    ...
    #define CPPCMS_BOOST_PP_LOCAL_MACRO(n) \
        CPPCMS_BOOST_IOSTREAMS_TEMPLATE_PARAMS(n, T) \
        explicit symmetric_filter( \
              int buffer_size CPPCMS_BOOST_PP_COMMA_IF(n) \
              CPPCMS_BOOST_PP_ENUM_BINARY_PARAMS(n, const T, &t) ) \
            : pimpl_(new impl(buffer_size CPPCMS_BOOST_PP_COMMA_IF(n) \
                     CPPCMS_BOOST_PP_ENUM_PARAMS(n, t))) \
            { } \
        /**/
    #define CPPCMS_BOOST_PP_LOCAL_LIMITS (0, CPPCMS_BOOST_IOSTREAMS_MAX_FORWARDING_ARITY)
    ...
    template<typename Sink>
    void close(Sink& snk, CPPCMS_BOOST_IOS::openmode)
    {
        if ((state() & f_write) != 0) {
...
#if !CPPCMS_BOOST_WORKAROUND(__DECCXX_VER, CPPCMS_BOOST_TESTED_AT(60590042)) 
    private:
#endif
...
private:
    ...
    bool flush(Sink& snk, mpl::true_)
    {
        ...
        std::streamsize result =
            cppcms_boost::iostreams::write(snk, buf().data(), amt);
    ...
    #define CPPCMS_BOOST_PP_LOCAL_MACRO(n) \
        CPPCMS_BOOST_IOSTREAMS_TEMPLATE_PARAMS(n, T) \
        impl( int buffer_size CPPCMS_BOOST_PP_COMMA_IF(n) \
              CPPCMS_BOOST_PP_ENUM_BINARY_PARAMS(n, const T, &t) ) \
            : SymmetricFilter(CPPCMS_BOOST_PP_ENUM_PARAMS(n, t)), \
...
CPPCMS_BOOST_IOSTREAMS_PIPABLE(symmetric_filter, 2)
