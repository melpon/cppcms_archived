...
#ifndef CPPCMS_BOOST_IOSTREAMS_GZIP_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_GZIP_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif

#include <cppcms_boost/config.hpp> // STATIC_CONSTANT, STDC_NAMESPACE, 
...
#include <cppcms_boost/config.hpp>               // Put size_t in std.
#include <cppcms_boost/detail/workaround.hpp>
#include <cppcms_boost/cstdint.hpp>              // uint8_t, uint32_t.
...
#include <cppcms_boost/iostreams/detail/ios.hpp> // failure.
#include <cppcms_boost/iostreams/operations.hpp>
#include <cppcms_boost/iostreams/device/back_inserter.hpp>
#include <cppcms_boost/iostreams/filter/zlib.hpp>
#include <cppcms_boost/iostreams/pipeline.hpp>     
#include <cppcms_boost/iostreams/putback.hpp>         
...
using namespace cppcms_boost::iostreams::zlib;
...
//
class gzip_error : public CPPCMS_BOOST_IOSTREAMS_FAILURE {
public:
    explicit gzip_error(int error)
        : CPPCMS_BOOST_IOSTREAMS_FAILURE("gzip error"),
...
public:
    ...
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    {
        if (!(flags_ & f_header_done)) {
            ...
            offset_ += cppcms_boost::iostreams::write(snk, header_.data() + offset_, amt);
    ...
    template<typename Sink>
    void close(Sink& snk, CPPCMS_BOOST_IOS::openmode m)
    {
        if (m == CPPCMS_BOOST_IOS::out) {
            try {

                // Close zlib compressor.
                base_type::close(snk, CPPCMS_BOOST_IOS::out);
...
private:
    ...
    {
        cppcms_boost::iostreams::put(next, static_cast<char>(0xFF & n));
        cppcms_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 8)));
        cppcms_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 16)));
        cppcms_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 24)));
    }

    void close_impl()
    {
        #if CPPCMS_BOOST_WORKAROUND(__GNUC__, == 2) && defined(__STL_CONFIG_H) || \
            CPPCMS_BOOST_WORKAROUND(CPPCMS_BOOST_DINKUMWARE_STDLIB, == 1) \
            /**/
...
CPPCMS_BOOST_IOSTREAMS_PIPABLE(basic_gzip_compressor, 1)
...
class CPPCMS_BOOST_IOSTREAMS_DECL gzip_header {
public:
    gzip_header() { reset(); }
...
class CPPCMS_BOOST_IOSTREAMS_DECL gzip_footer {
public:
    gzip_footer() { reset(); }
...
public:
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        while (result < n && state_ != s_done) {
            ...
            if (state_ == s_header) {
                int c = cppcms_boost::iostreams::get(peek);
                ...
                if (footer_.done()) {
                    ...
                    if (traits_type::is_eof(c)) {
                        ...
                        peek.putback(c);
                        base_type::close(peek, CPPCMS_BOOST_IOS::in);
                        state_ = s_start;
                        header_.reset();
                        footer_.reset();
                    }
    ...
    {
        try {
            base_type::close(src, CPPCMS_BOOST_IOS::in);
        } catch (const zlib_error& e) {
            state_ = s_start;
            header_.reset();
            footer_.reset();
    ...
    struct peekable_source {
        ...
        std::streamsize read(char* s, std::streamsize n)
        {
            ...
            if (offset_ < pbsize) {
                ...
                CPPCMS_BOOST_IOSTREAMS_CHAR_TRAITS(char)::copy(
            ...
            std::streamsize amt = 
                cppcms_boost::iostreams::read(src_, s + result, n - result);
            return amt != -1 ? 
                result + amt : 
                result ? result : -1;
        }
        bool putback(char c)
        {
            if (offset_) {
                ...
                return cppcms_boost::iostreams::putback(src_, c);
            }
        }
...
CPPCMS_BOOST_IOSTREAMS_PIPABLE(basic_gzip_decompressor, 1)
...
{
    cppcms_boost::iostreams::back_insert_device<std::string> out(footer_);
