...
#ifndef CPPCMS_BOOST_IOSTREAMS_ARRAY_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_ARRAY_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif

#include <cppcms_boost/config.hpp>         // BOOST_MSVC, make sure size_t is in std.
...
#if !CPPCMS_BOOST_WORKAROUND(CPPCMS_BOOST_MSVC, <= 1300)
# define CPPCMS_BOOST_IOSTREAMS_ARRAY_CTOR(name, ch) \
    template<int N> \
    CPPCMS_BOOST_PP_CAT(basic_, name)(ch (&ar)[N]) \
        : base_type(ar) { } \
    /**/
#else
# define CPPCMS_BOOST_IOSTREAMS_ARRAY_CTOR(name, ch)
#endif
#define CPPCMS_BOOST_IOSTREAMS_ARRAY(name, mode) \
    template<typename Ch> \
    struct CPPCMS_BOOST_PP_CAT(basic_, name) : detail::array_adapter<mode, Ch> { \
    private: \
        typedef detail::array_adapter<mode, Ch>  base_type; \
    public: \
        typedef typename base_type::char_type    char_type; \
        typedef typename base_type::category     category; \
        CPPCMS_BOOST_PP_CAT(basic_, name)(char_type* begin, char_type* end) \
            : base_type(begin, end) { } \
        CPPCMS_BOOST_PP_CAT(basic_, name)(char_type* begin, std::size_t length) \
            : base_type(begin, length) { } \
        CPPCMS_BOOST_PP_CAT(basic_, name)(const char_type* begin, const char_type* end) \
            : base_type(begin, end) { } \
        CPPCMS_BOOST_PP_CAT(basic_, name)(const char_type* begin, std::size_t length) \
            : base_type(begin, length) { } \
        CPPCMS_BOOST_IOSTREAMS_ARRAY_CTOR(name, Ch) \
    }; \
    typedef CPPCMS_BOOST_PP_CAT(basic_, name)<char>     name; \
    typedef CPPCMS_BOOST_PP_CAT(basic_, name)<wchar_t>  CPPCMS_BOOST_PP_CAT(w, name); \
    /**/
CPPCMS_BOOST_IOSTREAMS_ARRAY(array_source, input_seekable)
CPPCMS_BOOST_IOSTREAMS_ARRAY(array_sink, output_seekable)
CPPCMS_BOOST_IOSTREAMS_ARRAY(array, seekable)
#undef CPPCMS_BOOST_IOSTREAMS_ARRAY_CTOR
#undef CPPCMS_BOOST_IOSTREAMS_ARRAY
