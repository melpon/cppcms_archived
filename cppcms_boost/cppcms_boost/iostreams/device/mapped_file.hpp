...
#include <cppcms_boost/config.hpp>                   // make sure size_t is in std.
#include <cstddef>                            // size_t.
#include <string>                             // pathnames.
#include <utility>                            // pair.
#include <cppcms_boost/config.hpp>                   // BOOST_MSVC.
#include <cppcms_boost/detail/workaround.hpp>
#include <cppcms_boost/iostreams/close.hpp>
#include <cppcms_boost/iostreams/concepts.hpp>
...
#include <cppcms_boost/iostreams/detail/ios.hpp>     // openmode, failure
#include <cppcms_boost/iostreams/detail/path.hpp>
...
public:
    ...
    CPPCMS_BOOST_IOS::openmode         mode;  // Deprecated
...
#ifndef CPPCMS_BOOST_IOSTREAMS_NO_WIDE_STREAMS
    CPPCMS_BOOST_STATIC_ASSERT((!is_same<Path, std::wstring>::value));
#endif
...
class CPPCMS_BOOST_IOSTREAMS_DECL mapped_file_source : public mapped_file_base {
private:
    ...
    friend struct cppcms_boost::iostreams::operations<mapped_file_source>;
public:
    ...
    CPPCMS_BOOST_STATIC_CONSTANT(size_type, max_length = static_cast<size_type>(-1));
    ...
    explicit mapped_file_source( const Path& path,
                                 ...
                                 cppcms_boost::intmax_t offset = 0 );
...
private:
    ...
    cppcms_boost::shared_ptr<impl_type> pimpl_;
};

//------------------Definition of mapped_file---------------------------------//

class CPPCMS_BOOST_IOSTREAMS_DECL mapped_file : public mapped_file_base {
private:
    typedef mapped_file_source                      delegate_type;
    typedef delegate_type::safe_bool                safe_bool;
    typedef basic_mapped_file_params<detail::path>  param_type;
    friend struct cppcms_boost::iostreams::operations<mapped_file >;
...
public:
    ...
    CPPCMS_BOOST_STATIC_CONSTANT(size_type, max_length = delegate_type::max_length);
    ...
    explicit mapped_file( const Path& path,
                          CPPCMS_BOOST_IOS::openmode mode =
                              CPPCMS_BOOST_IOS::in | CPPCMS_BOOST_IOS::out,
                          size_type length = max_length,
                          stream_offset offset = 0 );
...
class CPPCMS_BOOST_IOSTREAMS_DECL mapped_file_sink : private mapped_file {
public:
    friend struct cppcms_boost::iostreams::operations<mapped_file_sink>;
...
mapped_file_source::mapped_file_source( 
    const Path& path, size_type length, cppcms_boost::intmax_t offset)
...
void mapped_file_source::open(const basic_mapped_file_params<Path>& p)
{
    ...
    if (params.flags) {
        if (params.flags != mapped_file::readonly)
            throw new CPPCMS_BOOST_IOSTREAMS_FAILURE("invalid flags");
    } else {
        if (params.mode & CPPCMS_BOOST_IOS::out)
            throw new CPPCMS_BOOST_IOSTREAMS_FAILURE("invalid mode");
        params.mode |= CPPCMS_BOOST_IOS::in;
    }
    open_impl(params);
}
...
template<typename Path>
mapped_file::mapped_file( 
    const Path& path, CPPCMS_BOOST_IOS::openmode mode, 
    size_type length, stream_offset offset )
...
mapped_file_sink::mapped_file_sink(
    ...
    cppcms_boost::intmax_t offset, mapmode flags )
...
void mapped_file_sink::open(const basic_mapped_file_params<Path>& p)
{
    ...
    if (params.flags) {
        ...
        if (params.mode & CPPCMS_BOOST_IOS::in)
            throw new CPPCMS_BOOST_IOSTREAMS_FAILURE("invalid mode");
        params.mode |= CPPCMS_BOOST_IOS::out;
    }
...
struct operations<mapped_file_source>
    : cppcms_boost::iostreams::detail::close_impl<closable_tag>
{
