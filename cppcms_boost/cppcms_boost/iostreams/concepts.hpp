...
#ifndef CPPCMS_BOOST_IOSTREAMS_CONCEPTS_HPP_INCLUDED
#define CPPCMS_BOOST_IOSTREAMS_CONCEPTS_HPP_INCLUDED
...
#include <cppcms_boost/iostreams/detail/default_arg.hpp>
...
template<typename Mode, typename Ch = CPPCMS_BOOST_IOSTREAMS_DEFAULT_ARG(char)>
struct device {
    ...
    void close()
    {
        ...
        CPPCMS_BOOST_STATIC_ASSERT((!is_convertible<Mode, two_sequence>::value));
    }

    void close(CPPCMS_BOOST_IOS::openmode)
    {
        using namespace detail;
        CPPCMS_BOOST_STATIC_ASSERT((is_convertible<Mode, two_sequence>::value));
    }
...
template<typename Mode, typename Ch = CPPCMS_BOOST_IOSTREAMS_DEFAULT_ARG(wchar_t)>
...
struct filter {
    ...
    template<typename Device>
    void close(Device&, CPPCMS_BOOST_IOS::openmode)
    {
        using namespace detail;
