...
//     (defined(__BORLANDC__) && (__BORLANDC__ <= 0x0581) ) )   &&
    ...
    || (CPPCMS_BOOST_WORKAROUND( _MSC_VER, <= 1300) )                  \
    )                                                           \
...
#include <cppcms_boost/date_time/locale_config.hpp> //set up locale configurations

//Set up a configuration parameter for platforms that have 
//GetTimeOfDay
#if defined(CPPCMS_BOOST_HAS_GETTIMEOFDAY) || defined(CPPCMS_BOOST_HAS_FTIME)
#define CPPCMS_BOOST_DATE_TIME_HAS_HIGH_PRECISION_CLOCK
#endif
...
#define CPPCMS_BOOST_DATE_TIME_OPTIONAL_GREGORIAN_TYPES
// these extensions are known to cause problems with gcc295
#if defined(__GNUC__) && (__GNUC__ < 3)
#undef CPPCMS_BOOST_DATE_TIME_OPTIONAL_GREGORIAN_TYPES
#endif

#if (defined(CPPCMS_BOOST_NO_INCLASS_MEMBER_INITIALIZATION) || CPPCMS_BOOST_WORKAROUND( __BORLANDC__,  CPPCMS_BOOST_TESTED_AT(0x581) ) )
#define CPPCMS_BOOST_DATE_TIME_NO_MEMBER_INIT
#endif
...
#if defined(__BORLANDC__) && defined(CPPCMS_BOOST_BCB_WITH_RW_LIB)
#define CPPCMS_BOOST_DATE_TIME_NO_WISTREAM_ITERATOR
#endif


// Borland v5.64 does not have the following in std namespace; v5.5.1 does
#if defined(__BORLANDC__) && defined(CPPCMS_BOOST_BCB_WITH_STLPORT)
#include <locale>
namespace std {
...
#define CPPCMS_BOOST_DATE_TIME_INCLUDE_LIMITED_HEADERS
#endif
...
#  define CPPCMS_BOOST_DATE_TIME_UNREACHABLE_EXPRESSION(x) __assume(false)
#elif !defined(CPPCMS_BOOST_NO_UNREACHABLE_RETURN_DETECTION)
// Call to a non-returning function should suppress the warning
#  if defined(CPPCMS_BOOST_NO_STDC_NAMESPACE)
namespace std {
    using ::abort;
}
#  endif // defined(BOOST_NO_STDC_NAMESPACE)
#  define CPPCMS_BOOST_DATE_TIME_UNREACHABLE_EXPRESSION(x) std::abort()
#else
// For other poor compilers the specified expression is compiled. Usually, this would be a return statement.
#  define CPPCMS_BOOST_DATE_TIME_UNREACHABLE_EXPRESSION(x) x
#endif
...
#ifdef CPPCMS_BOOST_HAS_DECLSPEC // defined in config system
   // we need to import/export our code only if the user has specifically
   // asked for it by defining either BOOST_ALL_DYN_LINK if they want all boost
   // libraries to be dynamically linked, or BOOST_DATE_TIME_DYN_LINK
   // if they want just this one to be dynamically liked:
#  if defined(CPPCMS_BOOST_ALL_DYN_LINK) || defined(CPPCMS_BOOST_DATE_TIME_DYN_LINK)
      // export if this is our own source, otherwise import:
#     ifdef CPPCMS_BOOST_DATE_TIME_SOURCE
#       define CPPCMS_BOOST_DATE_TIME_DECL __declspec(dllexport)
#     else
#       define CPPCMS_BOOST_DATE_TIME_DECL __declspec(dllimport)
#     endif  // BOOST_DATE_TIME_SOURCE
#  endif  // DYN_LINK
#endif  // BOOST_HAS_DECLSPEC
//
// if BOOST_WHATEVER_DECL isn't defined yet define it now:
#ifndef CPPCMS_BOOST_DATE_TIME_DECL
#  define CPPCMS_BOOST_DATE_TIME_DECL
#endif

//
// Automatically link to the correct build variant where possible. 
// 
#if !defined(CPPCMS_BOOST_ALL_NO_LIB) && !defined(CPPCMS_BOOST_DATE_TIME_NO_LIB) && !defined(CPPCMS_BOOST_DATE_TIME_SOURCE)
//
// Set the name of our library, this will get undef'ed by auto_link.hpp
// once it's done with it:
//
#define CPPCMS_BOOST_LIB_NAME cppcms_boost_date_time
//
...
#if defined(CPPCMS_BOOST_HAS_THREADS) 
#  if defined(_MSC_VER) || defined(__MWERKS__) || defined(__MINGW32__) ||  defined(__BORLANDC__)
     //no reentrant posix functions (eg: localtime_r)
#  elif (!defined(__hpux) || (defined(__hpux) && defined(_REENTRANT)))
#   define CPPCMS_BOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS
#  endif
#endif
