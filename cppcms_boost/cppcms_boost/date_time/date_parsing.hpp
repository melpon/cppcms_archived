...
#include <iterator>
#include <algorithm>
#include <cppcms_boost/tokenizer.hpp>
#include <cppcms_boost/lexical_cast.hpp>
#include <cppcms_boost/date_time/compiler_config.hpp>
#include <cppcms_boost/date_time/parse_format_base.hpp>

#if defined(CPPCMS_BOOST_DATE_TIME_NO_LOCALE)
  ...
  convert_to_lower(std::string inp)
  {
    ...
    month_str_to_ushort(std::string const& s) {
      if((s.at(0) >= '0') && (s.at(0) <= '9')) {
        return cppcms_boost::lexical_cast<unsigned short>(s);
      }
      else {
    ...
    parse_date(const std::string& s, int order_spec = ymd_order_iso) {
      ...
      typedef cppcms_boost::char_separator<char, traits_type> char_separator_type;
      typedef cppcms_boost::tokenizer<char_separator_type,
      ...
      for(tokenizer_iterator beg=tok.begin();
        ...
        switch(spec_str.at(pos)) {
          case 'y':
          {
            year = cppcms_boost::lexical_cast<unsigned short>(*beg);
            break;
          }
          case 'm':
          {
          ...
          case 'd':
          {
            day = cppcms_boost::lexical_cast<unsigned short>(*beg);
            break;
          }
        } //switch
      }
    ...
    parse_undelimited_date(const std::string& s) {
      ...
      cppcms_boost::offset_separator osf(offsets, offsets+3, false, false);

      typedef typename cppcms_boost::tokenizer<cppcms_boost::offset_separator,
                                        std::basic_string<char>::const_iterator,
      ...
      for(typename tokenizer_type::iterator ti=tok.begin(); ti!=tok.end();++ti) {
        unsigned short i = cppcms_boost::lexical_cast<unsigned short>(*ti);
        switch(pos) {
        case 0: y = i; break;
        case 1: m = i; break;
        case 2: d = i; break;
...
#if (defined(CPPCMS_BOOST_MSVC) && (_MSC_VER < 1300))
    ...
    from_simple_string_type(const std::basic_string<charT>& s){
      ...
      typedef typename cppcms_boost::char_separator<charT, traits_type> char_separator;
      typedef typename cppcms_boost::tokenizer<char_separator,
