...
namespace date_time {
  ...
  struct c_time {
    public:
#if defined(CPPCMS_BOOST_DATE_TIME_HAS_REENTRANT_STD_FUNCTIONS)
      ...
      static std::tm* localtime(const std::time_t* t, std::tm* result)
      {
        ...
        if (!result)
          cppcms_boost::throw_exception(std::runtime_error("could not convert calendar time to local time"));
        return result;
      }
      //! requires a pointer to a user created std::tm struct
      inline
      static std::tm* gmtime(const std::time_t* t, std::tm* result)
      {
        ...
        if (!result)
          cppcms_boost::throw_exception(std::runtime_error("could not convert calendar time to UTC time"));
        return result;
      }
