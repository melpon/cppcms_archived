...
#ifndef CPPCMS_BOOST_UNORDERED_DETAIL_NODE_HPP_INCLUDED
#define CPPCMS_BOOST_UNORDERED_DETAIL_NODE_HPP_INCLUDED
...
#define CPPCMS_BOOST_UNORDERED_BORLAND_BOOL(x) (bool)(x)
#else
#define CPPCMS_BOOST_UNORDERED_BORLAND_BOOL(x) x
#endif
...
namespace cppcms_boost { namespace unordered_detail {
    ...
    template <class A>
    inline CPPCMS_BOOST_DEDUCED_TYPENAME ungrouped_node_base<A>::node_ptr&
    ...
    template <class A>
    inline CPPCMS_BOOST_DEDUCED_TYPENAME grouped_node_base<A>::node_ptr&
    ...
    void grouped_node_base<A>::unlink_node(bucket& b, node_ptr n)
    {
        ...
        if(*pos != n) {
            ...
            // Remove from group
            if(CPPCMS_BOOST_UNORDERED_BORLAND_BOOL(next) &&
                get(next).group_prev_ == n)
            {
                get(next).group_prev_ = get(n).group_prev_;
            }
        }
        else if(CPPCMS_BOOST_UNORDERED_BORLAND_BOOL(next) &&
            get(next).group_prev_ == n)
        {
        ...
        node_ptr begin, node_ptr end)
    {
        ...
        if(*pos != begin) {
            ...
            // Remove from group
            if(CPPCMS_BOOST_UNORDERED_BORLAND_BOOL(end)) split_group(end);
        }
        else {
            node_ptr group1 = split_group(begin);
            if(CPPCMS_BOOST_UNORDERED_BORLAND_BOOL(end)) {
