...
#ifndef CPPCMS_BOOST_VARIANT_DETAIL_MOVE_HPP
#define CPPCMS_BOOST_VARIANT_DETAIL_MOVE_HPP
...
#if   defined(CPPCMS_BOOST_NO_ARGUMENT_DEPENDENT_LOOKUP)   \
 ||   CPPCMS_BOOST_WORKAROUND(__GNUC__, CPPCMS_BOOST_TESTED_AT(2))

// [Indicate that move_swap by overload is disabled...]
#define CPPCMS_BOOST_NO_MOVE_SWAP_BY_OVERLOAD

// [...and provide straight swap-by-move implementation:]
template <typename T>
inline void move_swap(T& lhs, T& rhs)
{
    T tmp( cppcms_boost::detail::variant::move(lhs) );
    lhs = cppcms_boost::detail::variant::move(rhs);
    rhs = cppcms_boost::detail::variant::move(tmp);
}
