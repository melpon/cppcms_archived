...
#ifndef CPPCMS_BOOST_FORMAT_IMPLEMENTATION_HPP
#define CPPCMS_BOOST_FORMAT_IMPLEMENTATION_HPP
...
// ---  basic_format implementation -----------------------------------------//
...
#if !defined(CPPCMS_BOOST_NO_STD_LOCALE)
        Ch fill = ( CPPCMS_BOOST_USE_FACET(std::ctype<Ch>, getloc()) ). widen(' ');
#else
        Ch fill = ' ';
#endif
        if(items_.size() == 0)
    ...
    clear () {
        ...
        CPPCMS_BOOST_ASSERT( bound_.size()==0 || num_args_ == static_cast<int>(bound_.size()) );
    ...
    clear_bind (int argN) {
        ...
        if(argN<1 || argN > num_args_ || bound_.size()==0 || !bound_[argN-1] ) {
            if( exceptions() & io::out_of_range_bit)
                cppcms_boost::throw_exception(io::out_of_range(argN, 1, num_args_+1 ) ); 
            else return *this;
        }
        bound_[argN-1]=false;
        clear();
    ...
    str () const {
        ...
        if( cur_arg_ < num_args_)
            if( exceptions() & io::too_few_args_bit )
                ...
                cppcms_boost::throw_exception(io::too_few_args(cur_arg_, num_args_)); 
        ...
        for(i=0; i < items_.size(); ++i) {
            ...
            if( item.argN_ == format_item_t::argN_tabulation) { 
                CPPCMS_BOOST_ASSERT( item.pad_scheme_ & format_item_t::tabulation);
...
#pragma warning(disable:4267)
        ...
        for(i=0; i < items_.size(); ++i) {
            ...
            sz += item.res_.size();
            if( item.argN_ == format_item_t::argN_tabulation)
                sz = max CPPCMS_BOOST_PREVENT_MACRO_SUBSTITUTION (sz,
...
namespace detail {
    ...
    bind_arg_body (basic_format<Ch, Tr, Alloc>& self, int argN, const T& val) {
        ...
        if(argN<1 || argN > self.num_args_) {
            if( self.exceptions() & io::out_of_range_bit )
                cppcms_boost::throw_exception(io::out_of_range(argN, 1, self.num_args_+1 ) );
            else return self;
        }
        if(self.bound_.size()==0) 
            self.bound_.assign(self.num_args_,false);
        else 
            CPPCMS_BOOST_ASSERT( self.num_args_ == static_cast<signed int>(self.bound_.size()) );
        ...
        CPPCMS_BOOST_ASSERT( self.cur_arg_ >= self.num_args_ || ! self.bound_[self.cur_arg_]);
        return self;
    }
    ...
    modify_item_body (basic_format<Ch, Tr, Alloc>& self, int itemN, T manipulator) {
        ...
        if(itemN<1 || itemN > static_cast<signed int>(self.items_.size() )) {
            if( self.exceptions() & io::out_of_range_bit ) 
                cppcms_boost::throw_exception(io::out_of_range(itemN, 1, static_cast<int>(self.items_.size()) ));
            else return self;
        }
