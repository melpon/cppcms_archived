...
#ifndef CPPCMS_BOOST_FORMAT_FUNCS_HPP
#define CPPCMS_BOOST_FORMAT_FUNCS_HPP
...
namespace cppcms_boost {
    ...
    namespace io {
         using ::cppcms_boost::str; // keep compatibility with when it was defined in this N.S.
    }   // - namespace io
    ...
    {
        typedef cppcms_boost::basic_format<Ch, Tr, Alloc>   format_t;
        if(f.items_.size()==0) 
            os << f.prefix_;
        else {
            if(f.cur_arg_ < f.num_args_)
                if( f.exceptions() & io::too_few_args_bit )
                    ...
                    cppcms_boost::throw_exception(io::too_few_args(f.cur_arg_, f.num_args_)); 
