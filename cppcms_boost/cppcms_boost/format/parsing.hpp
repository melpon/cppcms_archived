...
#ifndef CPPCMS_BOOST_FORMAT_PARSING_HPP
#define CPPCMS_BOOST_FORMAT_PARSING_HPP
...
#if ! defined( CPPCMS_BOOST_NO_LOCALE_ISDIGIT )
...
# else
                                      ...
                                      std::size_t pos, std::size_t size)
    {
        if(exceptions & io::bad_format_string_bit)
            cppcms_boost::throw_exception(io::bad_format_string(pos, size) );
    }
                                 ...
                                 unsigned char exceptions) 
    {
        ...
        using namespace cppcms_boost::io;
        typename String::size_type i1=0;
        int num_items=0;
        while( (i1=buf.find(arg_mark,i1)) != String::npos ) {
            if( i1+1 >= buf.size() ) {
                if(exceptions & bad_format_string_bit)
                    cppcms_boost::throw_exception(bad_format_string(i1, buf.size() )); // must not end in ".. %"
                else {
                  ++num_items;
                  break;
                }
...
#if !defined(CPPCMS_BOOST_NO_STRING_APPEND)
...
//  format :: parse(..)
    ...
    parse (const string_type& buf) {
        ...
        using namespace std;
#if !defined(CPPCMS_BOOST_NO_STD_LOCALE)
        const std::ctype<Ch> & fac = CPPCMS_BOOST_USE_FACET( std::ctype<Ch>, getloc());
#else
        ...
        while( (i1=buf.find(arg_mark,i1)) != string_type::npos ) {
            ...
            CPPCMS_BOOST_ASSERT(  static_cast<unsigned int>(cur_item) < items_.size() || cur_item==0);
        ...
        CPPCMS_BOOST_ASSERT(cur_item == num_items);
        ...
        if( !ordered_args) {
            if(max_argN >= 0 ) {  // dont mix positional with non-positionnal directives
                if(exceptions() & io::bad_format_string_bit)
                    cppcms_boost::throw_exception(io::bad_format_string(max_argN, 0));
