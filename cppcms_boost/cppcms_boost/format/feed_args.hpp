...
#ifndef CPPCMS_BOOST_FORMAT_FEED_ARGS_HPP
#define CPPCMS_BOOST_FORMAT_FEED_ARGS_HPP
...
#include <cppcms_boost/format/detail/msvc_disambiguater.hpp>
...
// the trick is in "boost/format/msvc_disambiguater.hpp"
    ...
    void put_head (CPPCMS_BOOST_IO_STD basic_ostream<Ch, Tr> & os, const T& x ) {
        disambiguater<Ch, Tr, T>::put_head(os, x, 1L);
    }
    template< class Ch, class Tr, class T> inline
    void put_last (CPPCMS_BOOST_IO_STD basic_ostream<Ch, Tr> & os, const T& x ) {
    ...
    template< class Ch, class Tr, class T> inline
    void put_head (CPPCMS_BOOST_IO_STD basic_ostream<Ch, Tr> &, const T& ) {
    }

    template< class Ch, class Tr, class T> inline
    void put_head( CPPCMS_BOOST_IO_STD basic_ostream<Ch, Tr> & os, const group1<T>& x ) {
    ...
    template< class Ch, class Tr, class T> inline
    void put_last( CPPCMS_BOOST_IO_STD basic_ostream<Ch, Tr> & os, const group1<T>& x ) {
        os << group_last(x.a1_); // this selects the last element
    }

#ifndef CPPCMS_BOOST_NO_OVERLOAD_FOR_NON_CONST 
    template< class Ch, class Tr, class T> inline
    void put_head( CPPCMS_BOOST_IO_STD basic_ostream<Ch, Tr> &, T& ) {
    }

    template< class Ch, class Tr, class T> inline
    void put_last( CPPCMS_BOOST_IO_STD basic_ostream<Ch, Tr> & os, T& x) {
        os << x ;
    }
#endif
            ...
            if(res_size == static_cast<size_type>(w) && w<=specs.truncate_ && !prefix_space) {
                ...
                if(static_cast<size_type>(w) <= tmp_size) { 
                    ...
                    res.assign(tmp_beg, i);
                                        ...
                                        CPPCMS_BOOST_ASSERT(d>0);
                    res.append(static_cast<size_type>( d ), oss2.fill());
                    res.append(tmp_beg+i, tmp_size-i);
                    CPPCMS_BOOST_ASSERT(i+(tmp_size-i)+(std::max)(d,(std::streamsize)0) 
                                 == static_cast<size_type>(w));
                    CPPCMS_BOOST_ASSERT(res.size() == static_cast<size_type>(w));
                }
            }
        }
        buf.clear_buffer();
    ...
    void distribute (basic_format<Ch,Tr, Alloc>& self, T x) {
        ...
        if(self.cur_arg_ >= self.num_args_)  {
            if( self.exceptions() & too_many_args_bit )
                cppcms_boost::throw_exception(too_many_args(self.cur_arg_, self.num_args_)); 
            else return;
        }
        for(unsigned long i=0; i < self.items_.size(); ++i) {
            if(self.items_[i].argN_ == self.cur_arg_) {
                put<Ch, Tr, Alloc, T> (x, self.items_[i], self.items_[i].res_, 
                                self.buf_, cppcms_boost::get_pointer(self.loc_) );
            }
        }
    }
